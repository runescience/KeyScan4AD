	...
	private void doIt() {
		...
		while (true) {
			...
			if (result.equals("empty"))
				System.out.println("\nYour next action? (getMAP, getROOm, getCoRRidor,rooMLIST,SEArch,quit,help)");
			else
				System.out.println("\nYour next action? (getMap, getRoom, getCorridor,roomlist,quit,help)");
			...
			if (containsIgnoreCase("quit", action)) {
				...
				System.out.println("Thanks for playing! type one of the following:\n"
						+ "map,room,corridor,roomlist,quit\n"
						+ "1,2,3,hello,treasure,quest,troll,orc,goblin,\n"
						+ "skel,fungi,rats,bats,swarm,cent,frogs,rats,\n"
						+ "verm,minion,boss,\n"
						+ "note ==>you can only certain options when the come up like: search\n"
						+ "note ==>1,2,3 means roll 1, 2, or 3 dice\n");
